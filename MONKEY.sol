...
contract MONKEY is ERC20, Ownable {
    ...
    MONKEYDividendTracker public dividendTracker;
    ...
    uint256 public immutable fixedSaleStartTimestamp = 1626737400; //July 19, 19:30 GMT-4, 2021
    ...
    uint256 public immutable tradingEnabledTimestamp = 1626737400; //July 19, 19:30 GMT-4, 2021
    ...
    constructor() public ERC20("MONKEY", "MONKEY") {
    	...
    	dividendTracker = new MONKEYDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MONKEY: The dividend tracker already has that address");

        MONKEYDividendTracker newDividendTracker = MONKEYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MONKEY: The new dividend tracker must be owned by the MONKEY token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MONKEY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MONKEY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MONKEY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MONKEY: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MONKEY: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MONKEY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MONKEY: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MONKEY: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "MONKEY: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "MONKEY: The fixed-sale is only available to certain participants at the start");
            }
...
contract MONKEYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("MONKEY_Dividend_Tracker", "MONKEY_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public override {
        require(false, "MONKEY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MONKEY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MONKEY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MONKEY_Dividend_Tracker: Cannot update claimWait to same value");
